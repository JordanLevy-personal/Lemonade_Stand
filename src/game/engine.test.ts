...
describe('multiplayer engine', () => {
  ...
  it('emits customer telemetry reasons and offer-score details for price rejection and sold-out outcomes', () => {
    ...
    expect(
      soldOut.room.simulation?.events.find((event) => event.outcome === 'soldOut')?.feedbackHintsByPlayerId,
    ).toBeUndefined()
  })

  it('attaches recipe feedback hints to purchased customer events for upgraded players', () => {
