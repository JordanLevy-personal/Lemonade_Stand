...
export interface PlayerDayPlanTelemetryRecord {
  ...
  marketEspionageOwnedBeforePlanning: boolean
  purchases: Inventory
  recipe: Recipe
  price: number
  submittedAt: string
...
export interface PlayerDayOutcomeTelemetryRecord {
  ...
  marketEspionageOwnedAfterResults: boolean
  cupsSold: number
  revenue: number
  satisfaction: number
  ...
  initialize(): void {
    ...
    this.database.exec(`
      ...
      create table if not exists player_day_records (
        ...
        market_espionage_owned_before_planning integer not null default 0,
        ...
        market_espionage_owned_after_results integer,
        cups_sold integer,
        revenue real,
        satisfaction real,
    ...
    this.ensureColumn(
      'player_day_records',
      'market_espionage_owned_after_results',
      'integer',
    )
  ...
  upsertPlayerDayPlan(record: PlayerDayPlanTelemetryRecord): void {
    this.requireDatabase().prepare(`
      insert into player_day_records (
        ...
        :marketEspionageOwnedBeforePlanning,
        :purchasesLemons,
        :purchasesSugar,
        :purchasesIce,
        :recipeLemons,
      ...
      on conflict(game_id, day_number, player_id) do update set
        ...
        market_espionage_owned_before_planning = excluded.market_espionage_owned_before_planning,
    ...
    `).run({
      ...
      marketEspionageOwnedBeforePlanning: record.marketEspionageOwnedBeforePlanning ? 1 : 0,
  ...
  upsertPlayerDayOutcome(record: PlayerDayOutcomeTelemetryRecord): void {
    this.requireDatabase().prepare(`
      ...
      set
        ...
        market_espionage_owned_after_results = :marketEspionageOwnedAfterResults,
        cups_sold = :cupsSold,
        revenue = :revenue,
    ...
    `).run({
      ...
      marketEspionageOwnedAfterResults: record.marketEspionageOwnedAfterResults ? 1 : 0,
